...
const PartnerCard = ({
  ...
  return (
    <div className="relative max-w-md w-full min-w-[192px]  h-48 overflow-hidden rounded-xl border border-gray-200 p-[1px] backdrop-blur-3xl">
      ...
      <div
        className={clsx(
          'inline-flex h-full w-full relative items-center justify-center rounded-xl px-3 py-1 text-sm font-medium backdrop-blur-3xl',
          light ? 'bg-foreground-heading' : 'bg-background'
        )}
      >
        <Image
          src={icon}
        ...
        <Image
          ...
          width={192}
          height={192}
          className="w-[50%] h-[50%] blur-3xl absolute top-0 opacity-50"
          quality={10}
        />
