...
const PersonCard = ({name, role, imageRoute, fact1, fact2, fact3, fact1col, fact2col, fact3col}: Props) => {
    return (
        <div className="p-4 rounded-xl w-[18.75rem] h-fit bg-gradient-to-br from-[#0D0E1B] to-[#0d0e1b00] grid gap-8 font-atyp-text font-normal group">
            <div className="rounded-[0.625rem] overflow-hidden relative w-[17rem] h-[21rem]">
                <div className="absolute opacity-0 z-20 group-hover:opacity-100 p-4 flex flex-col gap-4 duration-200">
                    <Fact fact={fact1} factcol={fact1col}/>
                    <Fact fact={fact2} factcol={fact2col}/>
                    <Fact fact={fact3} factcol={fact3col}/>
                </div>
                <Image src={imageRoute} alt={`Image of ` + name} width={2000} height={2000} className="group-hover:opacity-10 duration-200 z-10"></Image>
            </div>
