...
const TokenCard = ({className, name, ticker, icon, bgEffect}: Props) => {
  return (
    ...
    <div className={`flex flex-col justify-between items-start md:w-[27rem] md:h-[12rem] sm:w-[22rem] w-full h-[10rem] p-6 rounded-xl border border-[#fafbff12] bg-card-dark relative overflow-hidden group ${className}`}>
        ...
        <div className='absolute -right-16 -top-20'>
            <Image quality={80} src={bgEffect} aria-hidden alt='' unselectable='on' width={384} height={224} className='pointer-events-none select-none'></Image>
        </div>
    {/* </Link> */}
    </div>
  )
