...
const TokenCard = ({className, ticker, icon, url, bgColor, isLight}: Props) => {
    return (
        <Link href={url} target='_blank' className={`flex items-center justify-center h-20 sm:h-36 w-full sm:w-40 rounded-lg group overflow-hidden relative cursor-pointer ${bgColor} ${!isLight ? 'text-text-white' : 'text-text-black'}`}>
            ...
            <div className='absolute -bottom-5 group-hover:bottom-[3.75rem] transition-all duration-300 ease-in-out'>
